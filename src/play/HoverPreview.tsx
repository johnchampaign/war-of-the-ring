      ...
      {r?.siegeBox && (() => {
        ...
        return (
          <div style={{ marginTop: 4, borderTop: '1px solid #3a342a', paddingTop: 3 }}>
            ...
            {Object.entries(box.units).filter(([, u]) => u!.regular + u!.elite > 0).map(([n, u]) => (
              <div key={n} style={{ color: NATION_COLOR[n as Nation] ?? '#ccc', fontSize: 12, fontWeight: 600 }}>{cap(n)}: {u!.regular}R / {u!.elite}E</div>
            ))}
            {box.leaders > 0 && <div style={{ color: '#cfd8e6', fontSize: 12 }}>{box.leaders} Free Peoples Leader{box.leaders > 1 ? 's' : ''}</div>}
            {box.nazgul > 0 && <div style={{ color: '#e6857f', fontSize: 12 }}>{box.nazgul} Nazgûl</div>}
            {box.characters.length > 0 && <div style={{ color: '#d9c98a', fontSize: 12 }}>Characters: {box.characters.map(charName).join(', ')}</div>}
          </div>
        );
      })()}
    </div>
