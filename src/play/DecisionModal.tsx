...
// costs differs per card: Relentless Assault and Onslaught are paid in hits on your
// OWN units, Dread and Despair in points of Nazgûl Leadership. "How much will you
// pay for this card?" told the player neither (report 4z435k6d6z574g1u), so the
// prompt names the card and the currency.
function cardCostTitle(data: { card?: string; kind?: string } | undefined): string {
  const title = data?.card ? (CARD.get(data.card)?.combat?.title ?? cardName(data.card)) : 'This Combat card';
  if (data?.kind === 'nazgulLeadership') return `${title} — how many points of Nazgûl Leadership will you forfeit?`;
  if (data?.kind === 'selfHits') return `${title} — how many hits will you inflict on your own units?`;
...
function HuntDetail({ view, data, onExplain }: { view: GameState; data?: { damage?: number; reveal?: boolean; source?: string; noReduce?: boolean }; onExplain: () => void }) {
  if (!data) return null;
