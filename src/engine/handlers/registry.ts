...
export interface EventTarget { /** The route traced for a multi-region card move: the regions ENTERED, in order, ending at `to`. Omitted = the quiet way round (see moveAllUnits). */ path?: RegionId[]; from?: RegionId; to?: RegionId; region?: RegionId; nation?: Nation; companion?: string; mode?: 'move' | 'attack' | 'hide' | 'none' | 'recruit'; figure?: 'regular' | 'elite'; slot?: number; eye?: boolean; count?: number; move?: { units?: Partial<Record<Nation, { regular?: number; elite?: number }>>; leaders?: number; nazgul?: number; characters?: string[] } }
