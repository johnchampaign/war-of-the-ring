...
| D5 | Battle resolution (p.29-32) | Interactive: combat-card play each round, casualty selection, cease/retreat | `combat.ts` is an INTERACTIVE sub-machine: **combat-card play EVERY round** (each side, a real 'combatCard' PendingChoice gated by the card's **"Play if…" precondition** — `combatPrecondMet` covers the modelled cards' precondition patterns; `combatCards.ts` maps ~30 combat titles to roll/re-roll/max-dice/extra-attack/extra-hit/cancel/negate mods), casualty selection, cease/continue, and retreat are all real prompts. **Remaining simplifications:** effects resolve simultaneously via mods rather than by **initiative order** (a card's initiative only bites when both sides' effects collide — e.g. competing cancels); **per-effect-initiative cards** (Mûmakil's two timings) modelled as a single effect; a few intricate effects (specific Character elimination, forfeit-Leadership, retreat-as-card) approximated/omitted; retreat destination auto-picks; ≤5 round cap; unrecognized preconditions default to playable. | Initiative-ordered resolution + per-effect cards next. |
