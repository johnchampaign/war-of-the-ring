...
| D5 | Battle resolution (p.29-32) | Interactive: combat-card play, casualty selection, cease/retreat each round | `combat.ts` is now an INTERACTIVE sub-machine: casualty selection (Regulars-first vs Elites-first, prompted only when both present), the attacker's cease/continue, and the defender's retreat are real PendingChoice prompts. **Still deferred:** combat-card play *within* a battle (auto "no card"); retreat destination auto-picks the first free adjacent region; ≤5 round cap. | Combat cards in battle + chosen retreat region land with the combat-card-in-combat increment. |
