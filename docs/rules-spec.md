...
| D12 | Fellowship revealed by the Hunt (p.39) | On reveal, the FP moves the figure up to Progress regions (its choice; never ending in an FP City/Stronghold), resets Progress, flips to Revealed; **+1 Hunt tile per Shadow Stronghold the traced path crosses** | The figure now **moves on reveal** (`revealFellowship`) up to Progress toward Mordor (it previously didn't move at all — a bug), can't end in an FP-controlled City/Stronghold, and Progress resets + Revealed. **Deviations:** the FP doesn't pick the reveal path (we head toward Morannon — the near-universal intent), and the **extra Hunt tile per Shadow Stronghold on the path is not drawn yet**. The latter means reveal-advancement is currently penalty-free (it inflates the FP's ring quest); modelling it — together with making the path the FP's choice — is the planned follow-up. | partially closed (core move fixed; path-choice + Stronghold-hunt pending). |
