...
| D5 | Battle resolution (p.29-32) | Interactive: combat-card play each round, casualty selection, cease/retreat | `combat.ts` is an INTERACTIVE sub-machine: **combat-card play** (each side, at battle start, a real 'combatCard' PendingChoice — `combatCards.ts` maps ~30 combat titles to roll/re-roll/max-dice/extra-attack/extra-hit/cancel/negate mods), casualty selection, cease/continue, and retreat are all real prompts. **Simplifications:** one card per side per battle applied in round 0 (not one per round); combat-card *preconditions* not yet enforced (any card with a modelled effect is playable); a few intricate effects (specific Character elimination, forfeit-Leadership, retreat-as-card) approximated/omitted; chosen sequentially not simultaneously; retreat destination auto-picks; ≤5 round cap. | Per-round cards + preconditions + the omitted effects refine later. |
| D6 | Hunt damage (p.41-42) | FP chooses casualty vs Corruption; re-roll conditions | Now INTERACTIVE: `hunt.ts` prompts FP (PendingChoice 'huntDamage') to absorb as Corruption or lose the Guide / a random Companion (excess → Corruption); Guide reassigns (Gollum if none). Re-roll conditions (Shadow Stronghold / Army / Nazgûl in the region) modelled. **Still deferred:** the Guide's special abilities and "play on the table" damage-cancel cards before the choice; special-tile entry into the pool on Mordor. | Guide abilities + special-tile entry land with companion-abilities / Mordor increments. |
