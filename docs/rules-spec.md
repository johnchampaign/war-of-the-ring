...
| D7 | A few Event-card *riders* (the secondary "you may also…" clauses) | Optional follow-on moves / free card plays | Primary effect modelled faithfully; these specific optional riders are not | Low-value optional add-ons; the card's main effect is intact. **Rage of the Dunlendings** (sh-str-11): recruits 2 Isengard in a chosen free region adjacent to a Dunland — its optional consolidation *move* is not offered. **There Is Another Way** (fp-char-10): heals 1 and, with Gollum as Guide, auto-*hides* the Fellowship if revealed — the alternative *move* (which would trigger a Hunt) is not auto-taken. **The Ents Awake** (fp-char-19/20/21): rolls the Orthanc attack and, if the Army is destroyed, eliminates the Nazgûl (recycled) and Minions there — the optional "play another Character Event without an Action die" rider is not modelled. |

| D8 | Muster die recruiting (p.26) | One Muster die buys 2 Regulars (two Settlements) / 2 Leaders / 1 Regular + 1 Leader / 1 Elite | All single-Settlement bundles are now offered: **1 Regular**, **1 Elite**, **1 Regular + 1 Leader**, **2 Leaders** (the last requires an existing Faction unit; `recruit()` takes a `leader` count). The **2-Regulars-into-two-Settlements** split is not modelled as one die — a 1-Regular recruit stands in for it (always a legal lesser muster). | The two-region split needs a compound action; the single-Settlement bundles already capture most muster value and keep the action space tractable. |
| D9 | Army movement (p.27–28) | One Army die moves up to **2 different** armies one region each; a moving army may **split** (leave a rearguard) | One Army die moves **one** whole army one region; a **Character die** may move one army that contains a Leader/Nazgûl/Character (now modelled). Armies move as a whole stack — **no splitting / rearguard**, and **no two-army move** on a single Army die. | Both omitted features multiply the action space combinatorially (which units, which two armies); single-stack single-army moves keep the AI/search tractable and the soak stable. The Character-die army move (the common, high-value case) IS modelled. |
| D10 | Besieged Stronghold limits (p.31–32) | Garrison in the siege box capped at 5; can't muster-recruit into a besieged Settlement | Muster-recruiting into a besieged friendly Stronghold is already prevented (the enemy besieger makes `armySide` enemy, which `recruit()` rejects). The explicit **siege-box 5-figure cap** is not separately enforced. | Recruit-in-besieged is correctly blocked; the 5-cap is a rare edge given the 10 region stacking limit and is left as a minor simplification. |
